...
( function( $ ) {
	...
	$.fn.droploader = function( options ) {
		...
		const $uploadWrap  = $( this )
			.addClass( 'droploader' )
				.html( `
					...
					<label for="file"><strong>Choose a file</strong><span class="drophere"> or drag it here</span>.</label>
				` ),
		...
		if( isDragDropSupported ) {
			$uploadWrap
				.addClass( 'has-dragdrop' ) // letting the CSS part to know drag&drop is supported by the browser
